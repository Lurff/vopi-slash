...
const data = {
    ...
    description:"Sunucuya Katılan Kullanıcıların Belirlenen Kanala Resimli Hoşgeldin Mesajı Göndermesini Sağlar",
    ...
    execute(interaction) {
        ...
        const kontrol = db.fetch(`joined_member_channel_${interaction.guild.id}`)
        ...
        if(channel && deleted){
            ...
            db.set(`joined_member_channel_${interaction.guild.id}`,channel.id)
            interaction.reply({embeds:[embed("",`${channel} Adlı Kanal Başarılı Bir Şekilde Hoşgeldin Mesaj Kanalı Olarak Ayarlanmıştır`,"BLURPLE")]})

            const log = db.fetch(`log_${interaction.guild.id}`)
            if(!log) return;

            const Log = new MessageEmbed()
            .setTitle("Hoşgeldin Mesaj Kanalı Ayarlanmıştır")
            .setColor("#2F3136")
            .addFields([
            ...
            if(!kontrol) return interaction.reply({embeds:[embed("","Hoşgeldin Mesaj Kanalı Bulunmamaktadır","RED")]})

            db.delete(`joined_member_channel_${interaction.guild.id}`)
            ...
            const Log = new MessageEmbed()
            .setTitle("Hoşgeldin Mesaj Kanalı Sıfırlanmıştır")
            .setColor("#2F3136")
            .addFields([
